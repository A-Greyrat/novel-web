...
export function BinarizationFilter(gl, param, callback) {
    ...
    uniform vec3 u_colorBlack;
    void main() {
        vec4 color = texture(u_image, v_texCoord);
        float luminance = dot(color.rgb, vec3(0.2126, 0.7152, 0.0722));
        float binarized = step(u_threshold, luminance);
        vec3 colorWhite = u_colorWhite * binarized;
        vec3 colorBlack = u_colorBlack * (1.0 - binarized);
        fragColor = vec4(colorWhite + colorBlack, 1.0); 
    }
    `;
    ...
    param.colorWhite.forEach((v, i) => param.colorWhite[i] = Clamp(v, 0, 255) / 255);
    param.colorBlack.forEach((v, i) => param.colorBlack[i] = Clamp(v, 0, 255) / 255);
    ...
    return (srcTexture, dstFbo) => BlitTexture(gl, srcTexture, dstFbo, program, gl.drawingBufferWidth, gl.drawingBufferHeight, () => {
        ...
        gl.uniform3fv(gl.getUniformLocation(program, "u_colorWhite"), param.colorWhite);
        gl.uniform3fv(gl.getUniformLocation(program, "u_colorBlack"), param.colorBlack);
    });
}